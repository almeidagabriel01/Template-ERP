...
```bash
CONFIG=$(node "D:/DEV/ProOps/.claude/get-shit-done/bin/gsd-tools.cjs" state load)
```
...
```bash
REVIEW_CMD=$(node "D:/DEV/ProOps/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review_command --default "" 2>/dev/null)
```
...
2. **Load phase context from STATE.md:**
   ```bash
   STATE_STATUS=$(node "D:/DEV/ProOps/.claude/get-shit-done/bin/gsd-tools.cjs" state load 2>/dev/null | head -20)
   ```
...
```bash
node "D:/DEV/ProOps/.claude/get-shit-done/bin/gsd-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node "D:/DEV/ProOps/.claude/get-shit-done/bin/gsd-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
node "D:/DEV/ProOps/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
