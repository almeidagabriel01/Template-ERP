...
export const createMember = functions
  ...
  .https.onCall(async (data: CreateMemberInput, context) => {
    ...
    // ============================================
    // STEP 4: Check Plan Limits (Parallelized)
    // ============================================
    ...
    // 1. Prepare User Count Promise
    const countPromise = usersQuery.count().get();

    // 2. Prepare Limit Resolution Promise
    ...
    let limitPromise: Promise<number>;

    if (LEGACY_LIMITS[planId] !== undefined) {
      console.log(`[createMember] Using legacy limit for ${planId}`);
      limitPromise = Promise.resolve(LEGACY_LIMITS[planId]);
    } else {
      ...
      limitPromise = db
        ...
        .then((planSnap) => {
          if (planSnap.exists) {
            return planSnap.data()?.features?.maxUsers ?? 1;
          } else {
            console.warn(
              `[createMember] Plan ${planId} not found, defaulting to free limit.`
            );
            return 1;
          }
        });
    ...
    // 3. Await Both
    const [usersSnap, maxUsersVal] = await Promise.all([
      countPromise,
      limitPromise,
    ]);
    ...
    try {
      await db.runTransaction(async (transaction) => {
        ...
        if (companySnap.exists) {
          ...
          console.warn(
            `[createMember] Company/Tenant document ${masterData.tenantId} not found, skipping usage increment.`
          );
        }
      });
    } catch (err) {
