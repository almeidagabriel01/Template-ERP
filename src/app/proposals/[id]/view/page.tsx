      ...
      try {
        ...
        for (let i = 0; i < pageContainers.length; i++) {
          ...
          const pageCanvas = await html2canvas(pageElement, {
            ...
            onclone: (clonedDoc) => {
              console.log(
                `🔧 [PDF DEBUG PAGE ${i + 1}] Starting onclone fixes...`,
              );
              ...
              allImgs.forEach((img) => {
                ...
                if (
                  imgEl.alt === "EXTRA" ||
                  (imgEl.src && imgEl.src.includes("EXTRA"))
                ) {
                  // console.log(`Found EXTRA at image index ${idx}`);
                }
              });
              ...
              allImgs.forEach((img) => {
                ...
                if (
                  ...
                  const flexParent =
                    imgEl.closest('div[style*="display: flex"]') ||
                    imgEl.closest('div[style*="display:flex"]');

                  if (flexParent) {
                    const flexEl = flexParent as HTMLElement;
                    console.log(
                      `🔧 [FIX] Found flex parent for EXTRA ${extraCount}`,
                    );
              ...
              console.log(
                `🔧 [PDF DEBUG PAGE ${i + 1}] Fixed ${extraCount} EXTRA tags`,
              );

              // FIX 2: SISTEMA TITLE SPACING - Enhanced debugging
              let spacerCount = 0;
              console.log(
                `🔧 [DEBUG] Checking ${allH3s.length} h3 elements...`,
              );
              ...
              allH3s.forEach((h3, h3Index) => {
                ...
                const nextComputed = nextEl
                  ? window.getComputedStyle(nextEl)
                  : null;
                ...
                const isLargeTitle =
                  fontSize === "24px" ||
                  fontSize === "20px" ||
                  h3El.className.includes("text-xl");

                if (isLargeTitle) {
                  console.log(
                    `🔧 [FIX] Found large sistema title h3 ${h3Index + 1}`,
                  );
                  ...
                  if (nextEl && nextEl.tagName === "DIV") {
                    ...
                    const computedHeight =
                      window.getComputedStyle(divEl).height;
                    ...
                    if (
                      ...
                      console.log(
                        `🔧 [FIX] Enhancing spacer div after h3 ${h3Index + 1}`,
                      );
                      ...
                      console.log(
                        `⏭️ [SKIP] Not a spacer (has text: "${trimmedText.substring(0, 20)}")`,
                      );
                    }
                  }
                }
              ...
              console.log(
                `🔧 [PDF DEBUG PAGE ${i + 1}] Fixed ${spacerCount} spacers`,
              );

              console.log(
                `🔧 [PDF DEBUG PAGE ${i + 1}] Onclone fixes completed`,
              );
            },
          });
