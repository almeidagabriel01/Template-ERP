...
- **Global scope**: `D:/DEV/ProOps/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/DEV/ProOps/.claude/notes/`)
...
2. Glob `D:/DEV/ProOps/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/DEV/ProOps/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/DEV/ProOps/.claude/notes/` — works in any directory
