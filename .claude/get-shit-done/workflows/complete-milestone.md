...
```bash
node "D:/DEV/ProOps/.claude/get-shit-done/bin/gsd-tools.cjs" audit-open 2>/dev/null
```
...
Detect base branch:
```bash
BASE_BRANCH=$(node "D:/DEV/ProOps/.claude/get-shit-done/bin/gsd-tools.cjs" config-get git.base_branch 2>/dev/null || echo "")
