...
export function TypeSelectorStep({ type, onTypeChange }: TypeSelectorStepProps) {
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Escolha entre receita ou despesa</p>
                </div>
            </div>
          ...
          `}
            ...
            `}>
                        ...
                        <div className="text-center">
                            <p className={`font-bold text-xl ${type === "income" ? "text-green-600" : "text-foreground"}`}>
                                Receita
                            </p>
                            <p className="text-sm text-muted-foreground mt-1">Entrada de dinheiro</p>
                        </div>
                    </div>
                </button>
          ...
          `}
            ...
            `}>
                        ...
                        <div className="text-center">
                            <p className={`font-bold text-xl ${type === "expense" ? "text-red-600" : "text-foreground"}`}>
                                Despesa
                            </p>
                            <p className="text-sm text-muted-foreground mt-1">Saída de dinheiro</p>
                        </div>
                    </div>
                </button>
            </div>
        ...
        <div className="space-y-6">
            <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Informações básicas sobre a transação</p>
                </div>
            </div>
...
export function PaymentStep({ formData, onFormDataChange, onChange }: PaymentStepProps) {
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Configure como será pago este lançamento</p>
                </div>
            </div>
      ...
      `}>
          ...
          `}>
                    ...
                    <label htmlFor="isInstallment" className="flex-1 cursor-pointer">
                        <p className="font-semibold text-foreground text-lg">Parcelar Lançamento</p>
                        <p className="text-sm text-muted-foreground">Divida o valor em múltiplas parcelas</p>
                    </label>
                    <input
                        type="checkbox"
                        id="isInstallment"
...
interface ReviewStepProps {
    ...
    onClientChange: (data: { clientId?: string; clientName: string; isNew: boolean }) => void;
}

export function ReviewStep({ formData, onChange, onClientChange }: ReviewStepProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Summary Card */}
            <div className={`rounded-2xl border-2 overflow-hidden ${isIncome ? "border-green-500/30" : "border-red-500/30"
                }`}>
                <div className={`px-6 py-4 ${isIncome
                        ? "bg-gradient-to-r from-green-500/10 to-transparent"
                        : "bg-gradient-to-r from-red-500/10 to-transparent"
                    }`}>
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-sm text-muted-foreground">{isIncome ? "Receita" : "Despesa"}</p>
                            <p className="font-semibold text-lg">{formData.description || "Sem descrição"}</p>
                        </div>
                        <div className={`text-right ${isIncome ? "text-green-600" : "text-red-500"}`}>
                            <p className="text-sm">Valor Total</p>
                            <p className="text-2xl font-bold">
                                {isIncome ? "+" : "-"} R$ {parseFloat(formData.amount || "0").toFixed(2)}
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    {formData.isInstallment && (
                        <div className="p-4 rounded-xl bg-muted/30 border border-border/50">
                            ...
                            <p className="font-semibold">
                                {formData.installmentCount}x de R$ {(parseFloat(formData.amount || "0") / formData.installmentCount).toFixed(2)}
                            </p>
                        </div>
                    )}
                </div>
