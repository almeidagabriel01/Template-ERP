...
export const createClient = functions
  ...
  .https.onCall(async (data: CreateClientInput, context) => {
    ...
    if (isMaster) {
      ...
      if (!masterId) throw new functions.https.HttpsError("failed-precondition", "Erro de configuração de conta.");
      effectiveMasterId = masterId;
      masterRef = db.collection('users').doc(effectiveMasterId);
      const masterSnap = await masterRef.get();
      if (!masterSnap.exists) throw new functions.https.HttpsError("not-found", "Conta principal não encontrada.");
      masterData = masterSnap.data() as UserDoc;
    }

    const targetCompanyId = masterData.companyId || masterData.tenantId || userData.companyId || userData.tenantId;

    if (!targetCompanyId) {
        throw new functions.https.HttpsError("failed-precondition", "Configuração de conta inválida: companyId/tenantId ausente.");
    }

    // 4. Permission Check (MASTER/ADMIN/SUPERADMIN)
    // Only team managers can create clients directly via this function
    // Members might create clients via proposals, but usually we restrict main client DB to masters/admins
    // adjusting per requirement: "Criar CLIENTE" flow usually requires permissions.
    // Let's stick to restricting to MASTER/ADMIN for now as per "canManageTeam" or if they have permission?
    // The requirement says: "Cloud Functions são a ÚNICA autoridade... Validações de role (MASTER/MEMBER)"
    // Typically, MEMBERS can create clients if they have 'clients' page permission 'canCreate'.
    
    // REVISING: Check Page Permissions if MEMBER
    if (userData.role === 'MEMBER') {
       ...
       if (!permSnap.exists || !permSnap.data()?.canCreate) {
          throw new functions.https.HttpsError("permission-denied", "Sem permissão para criar clientes.");
       }
    }
    ...
    console.log(`[createClient] Limit: ${maxClients}, Usage: ${currentClients}`);
    ...
    try {
      const clientId = await db.runTransaction(async (transaction) => {
        ...
        if (maxClients >= 0 && freshUsage >= maxClients) {
           throw new functions.https.HttpsError("resource-exhausted", "Limite de clientes atingido.");
        }
        ...
        if (companySnap.exists) {
            ...
            console.warn(`[createClient] Company ${targetCompanyId} not found. Skipping usage increment.`);
        }

        return newClientRef.id;
      });
      
      return { success: true, clientId, message: "Cliente criado com sucesso!" };
      
    } catch (error) {
       console.error("Create Client Transaction Error:", error);
       if (error instanceof functions.https.HttpsError) throw error;
       throw new functions.https.HttpsError("internal", `Erro ao criar cliente: ${(error as Error).message}`);
    }
  });
