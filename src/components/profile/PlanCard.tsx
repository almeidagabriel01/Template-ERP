...
export function PlanCard({
  ...
  return (
    <div className={cn("relative group rounded-xl p-[1px] transition-transform duration-300",
      ...
      <Card
        className={cn(
          ...
          !isCurrent && !isPopular && "border border-border/40 hover:border-primary/50 hover:shadow-lg hover:-translate-y-1",
          isCurrent && "inner-shadow-primary",
        )}
      >
        {/* Subtle Inner Glow */}
        {isCurrent && <div className="absolute inset-0 bg-primary/5 pointer-events-none" />}
        {isPopular && <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/5 to-transparent pointer-events-none" />}
        ...
        <CardHeader className="pt-8 pb-6 px-6 z-10">
          <div className="flex items-center gap-3 mb-3">
            <div className={cn(
              ...
              isCurrent ? "bg-gradient-to-br from-primary to-primary/80 text-primary-foreground" : "bg-muted text-muted-foreground",
              isPopular && !isCurrent && "bg-gradient-to-br from-emerald-500 to-teal-600 text-white"
            )}>
              {isPopular ? <Sparkles className="w-6 h-6" /> : <Zap className="w-6 h-6" />}
            </div>
            <div>
              ...
              {isCurrent && <p className="text-xs font-medium text-primary mt-0.5">Seu plano atual</p>}
            </div>
          </div>
        ...
        <CardContent className="px-6 pb-8 flex-1 flex flex-col gap-6 z-10">
          ...
          <div className="mt-auto pt-2">
            {isCurrent ? (
              <Button className="w-full h-12 rounded-xl text-muted-foreground bg-muted/50 hover:bg-muted/70 border border-border/50" variant="ghost" disabled>
                Plano Ativo
              </Button>
            ) : isMaster ? (
              canUpgrade ? (
                <Button
                  className={cn(
                    ...
                    isPopular ? "bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white border-0" : "bg-primary hover:bg-primary/90 text-primary-foreground"
                  )}
                ...
                <Button
                  ...
                  disabled={isProcessing}
                >
                  {processingTier === plan.tier ? "Processando..." : "Mudar para este plano"}
                </Button>
              )
            ) : (
...
function FeatureRow({ label, value, highlight = false }: { label: string, value: string, highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between text-sm group/row">
      <div className="flex items-center gap-3 text-muted-foreground group-hover/row:text-foreground transition-colors">
        <div className={cn("w-1.5 h-1.5 rounded-full shrink-0", highlight ? "bg-primary" : "bg-muted-foreground/40")} />
        {label}
      </div>
      <span className={cn("font-semibold", highlight ? "text-foreground" : "text-muted-foreground")}>{value}</span>
    </div>
  );
}

function BooleanFeature({ label, available }: { label: string, available: boolean }) {
  return (
    <div className={cn("flex items-center gap-3 text-sm group/row", !available && "opacity-40 grayscale")}>
      <div className={cn(
        "w-5 h-5 rounded-full flex items-center justify-center shrink-0 border",
        available ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-600" : "border-muted bg-muted/50 text-muted-foreground"
      )}>
        <Check className="w-3 h-3" />
      </div>
      <span className={cn("transition-colors", available ? "text-foreground" : "text-muted-foreground line-through decoration-muted-foreground/50")}>
        {label}
      </span>
    </div>
  );
