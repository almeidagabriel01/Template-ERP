...
export function LinkedFieldSelector({
  ...
  const [parentType, setParentType] = React.useState<CustomFieldType | null>(
    null,
  );
  const [childType, setChildType] = React.useState<CustomFieldType | null>(
    null,
  );
  ...
  const updateParentItem = (entryId: string, parentItemId: string) => {
    onUpdate(
      entries.map((e) =>
        e.id === entryId ? { ...e, parentItemId, childItemIds: [] } : e,
      ),
    );
  };
  ...
  const getParentItem = (id: string) =>
    parentType?.items.find((i) => i.id === id);
      ...
      {entries.length === 0 ? (
        ...
        <div className="space-y-3">
          {entries.map((entry) => {
            ...
            const availableChildren = getChildItemsForParent(
              entry.parentItemId,
            );
                ...
                >
                  ...
                  <div className="flex-1" onClick={(e) => e.stopPropagation()}>
                    <select
                      ...
                      onChange={(e) =>
                        updateParentItem(entry.id, e.target.value)
                      }
                  ...
                  {entry.parentItemId && (
                    <span className="text-xs text-muted-foreground bg-muted px-2 py-1 rounded">
                      {entry.childItemIds.length} {childType.name.toLowerCase()}
                      (s)
                    </span>
                  )}
                ...
                {isExpanded && entry.parentItemId && (
                  <CardContent className="pt-0 pb-4 border-t bg-muted/30">
                    <Label className="text-xs text-muted-foreground mb-3 block">
                      Selecione {childType.name.toLowerCase()}s para{" "}
                      {parentItem?.label}:
                    </Label>

                    {availableChildren.length === 0 ? (
                      <p className="text-sm text-muted-foreground italic">
                        Nenhum {childType.name.toLowerCase()} disponível para
                        este {parentType.name.toLowerCase()}.
                      </p>
                    ) : (
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                        {availableChildren.map((item) => {
                          const isSelected = entry.childItemIds.includes(
                            item.id,
                          );
                          return (
                            <button
...
export function LinkedFieldsConfig({
  value,
  onChange,
}: LinkedFieldsConfigProps) {
