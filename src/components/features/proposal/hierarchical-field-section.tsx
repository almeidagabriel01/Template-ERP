...
export function HierarchicalFieldSection({ content, onUpdate }: HierarchicalFieldSectionProps) {
    const { tenant } = useTenant()
    const [fieldTypes, setFieldTypes] = React.useState<CustomFieldType[]>([])
    const [environmentType, setEnvironmentType] = React.useState<CustomFieldType | null>(null)
    const [systemType, setSystemType] = React.useState<CustomFieldType | null>(null)
    ...
    const getSystemItem = (itemId: string): CustomFieldItem | undefined => {
        return systemType?.items.find(i => i.id === itemId)
    }
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                    ...
                    >
                        <option value="">Selecione...</option>
                        {fieldTypes.filter(t => t.id !== content.environmentTypeId).map(type => (
                            <option key={type.id} value={type.id}>{type.name}</option>
                        ))}
                    </Select>
                </div>
                            ...
                            <Card key={entry.id} className="overflow-hidden">
                                ...
                                >
                                    ...
                                    {envItem?.image && (
                                        ...
                                        <img src={envItem.image} alt="" className="w-10 h-10 rounded object-cover" />
                                    )}
                                    ...
                                    <div className="flex-1">
                                        <Select
                                            ...
                                            onChange={(e) => {
                                                e.stopPropagation()
                                                updateEntry(entry.id, { environmentItemId: e.target.value })
                                            }}
                                            onClick={(e) => e.stopPropagation()}
                                            className="font-medium"
                                        >
                                            <option value="">Selecione {environmentType.name}...</option>
                                    ...
                                    <span className="text-xs text-muted-foreground">
                                        {entry.systemItems.length} {systemType.name.toLowerCase()}(s)
                                    </span>
                                ...
                                {isExpanded && entry.environmentItemId && (
                                    <CardContent className="pt-0 pb-3 border-t bg-muted/30">
                                        <Label className="text-xs text-muted-foreground mb-2 block">
                                            Selecione os {systemType.name.toLowerCase()}s para {envItem?.label}:
                                        </Label>
