...
export const updateProposal = functions
  ...
  .https.onCall(async (data: UpdateProposalInput, context) => {
    ...
    // 2. Parallel Fetch (User, Proposal, Permission)
    ...
    if (proposalData?.tenantId !== tenantId) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "Esta proposta não pertence a sua organização."
      );
    }
    ...
    const isMaster =
      ...
      role === "ADMIN" ||
      role === "WK" ||
      (!userData.masterId && userSnap.data()?.subscription);
    ...
    if (updateData.clientEmail !== undefined)
      safeUpdate.clientEmail = updateData.clientEmail;
    if (updateData.clientPhone !== undefined)
      safeUpdate.clientPhone = updateData.clientPhone;
    if (updateData.clientAddress !== undefined)
      safeUpdate.clientAddress = updateData.clientAddress;
    if (updateData.validUntil !== undefined)
      safeUpdate.validUntil = updateData.validUntil;
    if (updateData.status !== undefined) safeUpdate.status = updateData.status;
    if (updateData.products !== undefined)
      safeUpdate.products = updateData.products;
    if (updateData.sistemas !== undefined)
      safeUpdate.sistemas = updateData.sistemas;
    if (updateData.discount !== undefined)
      safeUpdate.discount = updateData.discount;
    if (updateData.notes !== undefined) safeUpdate.notes = updateData.notes;
    if (updateData.customNotes !== undefined)
      safeUpdate.customNotes = updateData.customNotes;
    if (updateData.sections !== undefined)
      safeUpdate.sections = updateData.sections;
    if (updateData.pdfSettings !== undefined)
      safeUpdate.pdfSettings = updateData.pdfSettings;
