  ...
  .https.onCall(async (data: DeleteMemberInput, context) => {
    ...
    try {
        ...
        if (err.code === 'auth/user-not-found') {
            console.warn(`[deleteMember] Auth user ${memberId} already missing, proceeding with DB cleanup.`);
        } else {
             console.error(`[deleteMember] Failed to delete auth user`, err);
             throw new functions.https.HttpsError("internal", "Erro ao remover acesso do usuário.");
        }
    }
    ...
    try {
        ...
        console.error(`[deleteMember] Transaction failed`, err);
         throw new functions.https.HttpsError("internal", "Erro ao remover dados do membro.");
    }
